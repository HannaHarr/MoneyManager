...
namespace MoneyManagerModel
{
    public static class InitialData
    {
        ...
        static InitialData()
        {
            using (var sr = new StreamReader(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, 
                "initial_data.json")))
            {
        ...
        private class DateTimeConverter : JsonConverter<DateTime>
        {
                ...
                JsonSerializerOptions options)
            {
                ...
                var date = reader.GetString().Split(dot);

                return new DateTime(int.Parse(date[2]), int.Parse(date[1]), int.Parse(date[0]));
            }

            public override void Write(Utf8JsonWriter writer, DateTime value, JsonSerializerOptions options) 
                => writer.WriteStringValue(value.Day.ToString() + dot + value.Month.ToString() + dot 
                    + value.Year.ToString());
        }
    }
}
